...
-- | Statically gated subroutines, one per rung of the hierarchy. The proof
-- comes from Satisfies; IsAtleast<Con> is generated by deriveOrdRole (one alias
-- per constructor). A gate must discharge its own rung /and every rung below
-- it/, so an Admin handler can call all three with no re-check.
viewProfile :: (IsAtleastViewer r) => RoleAuth r -> String
viewProfile auth = "viewed by " <> rName auth

editPost :: (IsAtleastEditor r) => RoleAuth r -> String
editPost auth = "edited by " <> rName auth
...
roleServer = viewerH :<|> editorH :<|> adminH
  where
    -- Each handler calls every subroutine its gate subsumes. The weakening is
    -- what is under test: only viewerH's call is an exact match on the gate's
    -- own role, the other three discharge strictly weaker requirements.
    viewerH :: Satisfies 'Viewer RoleAuth -> Handler String
    viewerH (Satisfies UserRoleProof auth) = pure (viewProfile auth)

    editorH :: Satisfies 'Editor RoleAuth -> Handler String
    editorH (Satisfies UserRoleProof auth) = pure (viewProfile auth <> "; " <> editPost auth)

    adminH :: Satisfies 'Admin RoleAuth -> Handler String
    adminH (Satisfies UserRoleProof auth) =
      pure (viewProfile auth <> "; " <> editPost auth <> "; " <> banUser auth)
    ...
    with roleApp $ do
      ...
      describe "hierarchical: /admin (requires Admin)" $ do
        ...
        it "Admin  -> 200, ban proof flows through" $
          roleReq "admin" "/admin"
            `shouldRespondWith` "\"viewed by Sandy; edited by Sandy; banned by Sandy\"" {matchStatus = 200}
        it "no auth -> 401" $ get "/admin" `shouldRespondWith` 401

      -- A gate's proof must discharge every requirement at or below its own
      -- role. Regression guard: these bodies only render if the weaker
      -- IsAtleast<Con> constraints were released by the proof, which needs
      -- transitivity of (<=) that GHC's solver cannot do on its own.
      describe "hierarchy weakening: a gate subsumes every rung below it" $ do
        it "Viewer gate discharges IsAtleastViewer" $
          roleReq "viewer" "/viewer" `shouldRespondWith` "\"viewed by Reed\"" {matchStatus = 200}
        it "Editor gate discharges IsAtleastViewer and IsAtleastEditor" $
          roleReq "editor" "/editor"
            `shouldRespondWith` "\"viewed by Lyxia; edited by Lyxia\"" {matchStatus = 200}
